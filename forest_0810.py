...
df1 = pd.read_excel(r'E:\test\fenlei4_Train.xlsx')
# print(df1.describe())
...
X_train, X_test, y_train, y_test = train_test_split(X, y, stratify=df1['TreeType'])
print('=' * 100)
...
df_pred = pd.read_excel(r'E:\test\fenlei4_PredictData.xlsx')
dataPred_features = df_pred[['blue', 'green', 'nir', 'red', 'NDVI', 'GNDVI', 'LCI', 'NDRE',
                             'elev_min', 'elev_percentile_99th',
                             'density_metrics[6]', 'density_metrics[8]', 'int_cv', 'int_max', 'int_skewness',
                             'Area']]
y_pred_pred = forest.predict(dataPred_features)
df_pred['TreeType'] = y_pred_pred
df_aaa = pd.DataFrame(df_pred)
pd.DataFrame(df_aaa).to_excel(r'E:\test\fenlei4_PredictData_type.xlsx', sheet_name='Sheet1', index=False, header=True)
# print(df_pred)
